...
async def find_next_change(
    ...
    if lo >= hi:
        return None
    diff_hi = await get_diff(async_substrate, wallet, hi)
    if diff_hi == base_diff:
        return None
    # diff(hi) != base_diff, so there is at least one change in (lo, hi]
    while lo + 1 < hi:
        mid = (lo + hi) // 2
        diff_mid = await get_diff(async_substrate, wallet, mid)
...
async def find_all_diff_changes(
    ...
    base_diff = await get_diff(async_substrate, wallet, 0)
    changes.append((0, base_diff))
    lo = 0
    while lo < max_tip_rao:
        next_tip = await find_next_change(
            async_substrate, wallet, lo, max_tip_rao, base_diff
        )
        if next_tip is None:
            break
        base_diff = await get_diff(async_substrate, wallet, next_tip)
...
async def main():
    async with AsyncSubstrateInterface(url=settings.NETWORK) as async_substrate:
        ...
        changes = await find_all_diff_changes(
            async_substrate, wallet, max_tip_rao=1_024_001 * 4
        )
