...
- **Fast stake/unstake (MevShield):** Uses Bittensor wallets named in `bt_utils/constants.py` (`DELETEGATE_1`, `DELETEGATE_2`; e.g. `"soon"`, `"soon_2"`). Create these under `~/.bittensor/wallets/` and fund them; do not exceed 2 TAO per delegate as above. Intent is encoded by calling **MevShield.announce_next_key** with tip = encoded(netuid, amount, etc.), even if that extrinsic fails. The contract's per-delegate base fees are the inclusion fee of that call; they are computed live from the chain (`bt_utils.fast_stake_unstake.compute_base_fees_rao`) and written on-chain via `setBaseFeesRao` at deploy time or with `scripts/set_fee.py`, rather than hardcoded.
